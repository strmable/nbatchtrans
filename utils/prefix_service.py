...
# [00001] 형식 매칭 (줄 시작, 5자리 0패딩 숫자, 대괄호)
...
class PrefixService:
    ...
    def add_prefixes_to_chunk(
        ...
        logger.debug(
            f"프리픽스 추가 완료: {prefix_counter}개 줄에 번호 부착 "
            f"(전체 {len(lines)}줄, global_offset={global_line_offset})"
        )
    ...
    def parse_prefixed_translation(self, translated_text: str) -> dict[int, str]:
        ...
        logger.debug(f"프리픽스 파싱 완료: {len(result)}개 번역 항목 추출")
        return result
    ...
    def reconstruct_output(
        ...
        - 번역 누락 줄 → @offset::줄번호 translate missing, original text "원문"
        ...
        if missing_count > 0:
            logger.warning(
                f"번역 누락 {missing_count}개 줄 → 원문 대체 및 @offset 주석 추가"
            )
